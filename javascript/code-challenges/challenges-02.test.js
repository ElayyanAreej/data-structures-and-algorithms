...
const extractAbilities = (arr) => {
  // Solution code here...
  let abilityNameArr=arr.map((item,idx)=>{
    return item.ability.name;
  });
  return abilityNameArr;
};
...
const extractStats = (arr) => {
  ...
  arr.forEach((item)=>{
    function Object() {
      this.name = `"${item.stat.name}"`;
      this.total = item.effort+item.baseStat;
      iniarr.push(this);
    }
    let obj=new Object();
    console.log(iniarr);}
