...
// if(item.includes(forbiddenValues[i])){
// console.log(item.includes(forbiddenValues[i]));
...
const getStatName = (arr, minBaseStat) => {
  // Solution code here...
  let newArry = getBaseStatGreaterThan(arr, minBaseStat);
  let newArry2 = newArry.map((item) => {
...
const getCharactersWithoutChildren = (arr) => {
  ...
  //   let newArry=arr.filter((item)=>{
  //     if( item.indexOf(Object.keys('children'))=== -1){
  //       return true;
  //     }}
  //   );
  //   return newArry;
  ...
  let newArry = arr.filter((item) => {
    let propName = Object.getOwnPropertyNames(item);
...
const evenOddNumericValues = (arr) => {
  ...
  let evenOddArry = newArry.map((item) => {
    if (item % 2 === 0) {
      return 'even';
    }
    else {
