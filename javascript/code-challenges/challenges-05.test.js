...
const addValues = (arr) => {
  // Solution code here...
  let newArry = arr.reduce((acc, val, idx) => {
    acc += val;
    return acc;
  }, 0);
  return newArry;
...
const addPurchases = (arr) => {
  // Solution code here...
  let totalPurchased = arr.reduce((acc, val, idx) => {
...
const returnNames = (arr) => {
  // Solution code here...
  let starWarsNames = arr.reduce((acc, val, idx) => {
    acc[idx] = val.name;
    return acc;
  }, []);
...
const reversedString = (str) => {
  ...
  let revArr = stringArr.reduce((acc, val, idx) => {
    idx = (stringArr.length - idx - 1);
    console.log(idx, stringArr[idx]);
    acc.push(stringArr[idx]);
    return acc;
  }, []);

  let revStr = revArr.reduce((acc, val, idx) => {
    acc += val;
    return acc;
  }, '');
