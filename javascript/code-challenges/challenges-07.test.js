...
const listFoods = (recipe) => {
  ...
  ingredients.forEach(item => {
    ...
    initialResult.push(item.slice(idx + 1, item.length));
  });


  let initialResult2 = [];
  initialResult.forEach(item2 => {
    let idx = item2.indexOf(' ');
    initialResult2.push(item2.slice(idx, item2.length));
  });


  initialResult2.forEach(item2 => {
    result.push(item2.slice(1, item2.length));
  });
