...
const checkValues = (obj, value) => {
  // Solution code here...
  let valuesArry = Object.values(obj);
  let result = valuesArry.find((item) => {
    if (item === value) {
      return item;
    }
  });
...
const updateNumbers = (obj) => {
  ...
  for (let i = 0; i < propArry.length; i++) {
    newArry.push(propArry[i] + ": " + valuesArry[i]);
  }
  return newArry;
};
...
const hasChildrenValues = (arr, character) => {
  // Solution code here...
  // to get obj has children
  let childrenObjArr = arr.filter((item) => {
  ...
  //   let childrenArrInitial = Object.values(childrenObjArr[i].children);
  //   childrenArr.push(childrenArrInitial);
  // }
  ...
  //   result = childrenArr[i].find((child) => {
  //     console.log(child);
  //     if (child === character) {
  //       return child;
  //     }
