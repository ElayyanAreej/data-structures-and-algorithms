...
function transformToLis(obj) {
  ...
  for (let i = 0; i < arrKeys.length; i++) {
    arr.push("<li>" + arrKeys[i] + ": " + arrValues[i] + "</li>");
  }
  return arr;
}
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let divisonByFiveArr = input.map((item) => {
    let newArr = item.filter((item2, indx) => {
      if (item2 % 5 === 0) {
        return true;
      }
    });
  ...
  console.log(divisonByFiveArr);

  let powerOfFiveArr = divisonByFiveArr.map((item) => {
    console.log(item);
    let newArr = item.map((item2) => {
      return Math.pow(2, item2);
    });

    return newArr;
  });
  return powerOfFiveArr;
  // let divisonByFiveArr = input.map((item) => {
  //   let newArr = item.filter((item2, indx) => {
  //     if (item2 % 5 === 0) {
  //       return true;
...
let findMaleAndFemale = (data) => {
  ...
  let strResult = "";
  namesArr.forEach((item, idx) => {
    if (namesArr.length - 1 === idx) {
      strResult = strResult + item;
    } else {
      strResult = strResult + item + " and ";
    }
  });
