...
const firstLetters = (arr) => {
  ...
  // str.charAt(0)
  arr.forEach((item) => {
    firstLettersArr.push(item.charAt(0));
  });
...
const findHappiness = (arr) => {
  // Solution code here...
  let findHappinessArr = arr.filter((item) => {
    if (item.includes(":)")) {
      return true;
    }
  });
...
const standardizePhoneNumbers = (arr) => {
  ...
  let arr1 = [];
  arr.forEach((item) => {
    arr1.push(item.slice(1, 4));
  });

  let arr2 = [];
  arr.forEach((item) => {
    arr2.push(item.slice(6, 9));
  });

  let arr3 = [];
  arr.forEach((item) => {
    arr3.push(item.slice(10));
  });

  let standardizePhoneNumbersArr = [];
  for (let i = 0; i < arr.length; i++) {
    standardizePhoneNumbersArr.push(arr1[i] + arr2[i] + arr3[i]);
  }
  return standardizePhoneNumbersArr;
};
...
const onlyOddChars = (str) => {
  ...
  for (let i = 0; i < str.length; i++) {
    if (i % 2 !== 0) {
      oddCharArr.push(str.charAt(i));
    }
    // console.log(str.charAt(i));
    // oddCharArr.push(str.charAt(i));
  }
  let oddStr = oddCharArr.join("");
  return oddStr;
};
